...
@section('body')
    <x-card-component title='Modify User Information'>
        <form action={{ route('users.update', $user) }} enctype="multipart/form-data" method='POST'>
            ...
            <p class="text-xs text-base-content/60 mt-1 max-w-xs">JPEG, PNG, GIF, or SVG, up to 2MB. Square images work best since they're displayed in a circle.</p>
            @error('image')
                <p class="text-error text-sm mt-1 max-w-xs">{{ $message }}</p>
            @enderror
            <div class="mb-5"></div>
